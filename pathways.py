    ...
    try:
        ...
        for i in range(0, len(body)):
            ...
            query = "insert into Pathways_Table(pathway, search_url, image_url, category, Drugs) \
                values ('"+pathway+"', '"+search_url+"', '"+image_url+"', '"+category+"', '" \
                +Drugs+"')"
        ...
        file.write(f"Page : {search_url}: {e}\n")
        file.close()     
def next_page(pathway, search_url, image_url, category, Drugs, svg_filename, svg_url):
    ...
    try:
        ...
        print(f"{pathway}: {e}")
        file = open("Errors.txt","a+")
        file.write(f"Next Page : {pathway}: {image_url}: {e}\n")
        file.close()   
...
def download_img(svg_filename, svg_url):
    try:
        ...
        print(f"{svg_url}: {e}")
        file = open("Errors.txt","a+")
        file.write(f"Image : {pathway}: {image_url}: {svg_url}: {e}\n")
        file.close()   
